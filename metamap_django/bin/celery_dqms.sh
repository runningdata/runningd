...
function check() {
    lines=`ps -ef |grep celery | grep dqms | grep ${1} | wc -l`
    if [[ $lines > 0 ]]; then
        echo "${lines}: celery ${1} still running..."
        ps -ef |grep celery | grep dqms | grep ${1}
        return ${lines}
    else
        echo "dqms celery ${1} has been killed"
        return 0
    fi
}
...
###  停止所有celery指定进程
function stop() {
    pid=`ps -ef | grep celery |  grep dqms | grep ${1} | awk '{if($3 == '1') print $2}'`
    if [[ $pid > 0 ]]; then
...
/server/xstorm/bin/python manage.py celery multi start will_dqms -A dqms \
  -Q dqms \
