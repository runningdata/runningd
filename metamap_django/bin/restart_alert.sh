...
function check_gunicorn() {
    lines=`ps -ef | grep alert_settings | grep -c  /server/xstorm/bin/gunicorn`
    if [[ $lines > 1 ]]; then
        echo "${lines} gunicorns still running..."
        ps -ef |grep alert_settings
        return ${lines}
    else
...
###  2. 停止所有gunicorn进程   ####
#################################
pid=`ps -ef | grep gunicorn | grep alert_settings | awk '{if($3 == '1') print $2}'`
if [[ $pid > 0 ]]; then
...
echo "##########   log  ############"
sleep 3s
tail -20 /tmp/running_alert_gunicorn_error.log